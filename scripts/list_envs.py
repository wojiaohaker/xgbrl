...
All the environments are registered in the `xgbrl` extension. They start
...
import xgbrl.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import xgbrl.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~xgbrl.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from xgbrl.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `xgbrl` extension."""
    ...
    if args_cli.show_presets:
        from xgbrl.utils.preset_cli import enumerate_task_presets
